...
For additional files from additional sessions, it is basically the same process of putting the downloaded docx file in the working directory (normaly the same folder as the script) and customising the name of the file being read in to match, and the name of the variable being stored under. So, for example the read the data in block early on in the above might (for day 2 morning) become
...
Of course, once you start adding multiple sessions you might want to add a variable for which session they are from, if working with more than one.
...
Since having a session variable means that the data can be combined into a singly data set.
